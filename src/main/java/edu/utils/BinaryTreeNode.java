...
public class BinaryTreeNode {
    ...
    List<BinaryTreeNode> childNodes;

    public BinaryTreeNode(String data, BinaryTreeNode parentNode) {
        this.data = data;
        this.parentNode = parentNode;
        this.childNodes = new ArrayList<BinaryTreeNode>();
    }

    public void addNode(String data) {
        //TODO check to what tree branch add new node -- needs getHeight
        if (childNodes.size() < MAX_CHILDREN_COUNT) {
            childNodes.add(new BinaryTreeNode(data, this));
        } else {
    ...
    public int[] getSortedHeights(List<BinaryTreeNode> childNodes) {
        int[] heights = new int[childNodes.size()];
        for (int i = 0; i < childNodes.size(); i++) {
            heights[i] = childNodes.get(i).getHeight();
        }
    ...
    public int size() {
        if (childNodes != null) {
            ...
            for (BinaryTreeNode childNode : childNodes) {
